...
import com.example.examples.common.Constants
import com.example.examples.common.Resource
import com.example.examples.domain.use_case.get_coin.GetCoinUseCase
    ...
    private fun getCoins(coinId: String) {
        getCoinUseCase(coinId = coinId).onEach { result ->
            when (result) {
                ...
                is Resource.Loading -> {
                    _state.value = CoinDetailState(istLoading = true)
                }
            }
